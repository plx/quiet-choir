...
export const meta = {
  ...
  description:
    'Find dead-code candidates via three search modalities, cross-check each with a default-to-keep skeptic, optionally remove and verify',
  whenToUse:
    'Codebase hygiene: finding provably-unused exports, files, and dependencies without breaking dynamic or reflective consumers',
  phases: [
...
async function run(ctx: WorkflowContext, args: z.infer<typeof input>) {
  ...
  async function execute() {
    const CANDIDATES_SCHEMA = z
      .object({
        candidates: z.array(
          z
            .object({
              identifier: z.string().describe('function/class/file/dependency name'),
              file: z.string(),
              kind: z.enum(['function', 'class', 'file', 'dependency', 'branch', 'config']),
              evidence: z.string().describe('why this looks dead'),
            })
            .catchall(z.json()),
        ),
      })
    ...
    const VERDICT_SCHEMA = z
      .object({
        alive: z.boolean().describe('true if ANY plausible living reference exists'),
        reasoning: z.string(),
        referenceFound: z.string().describe('where the living reference is, if alive').optional(),
      })
      .catchall(z.json());
    ...
    port.log(
      `${candidates.length} unique candidates from ${sweeps.filter(Boolean).length} modalities`,
    );

    if (candidates.length > maxCandidates) {
      // Corroborated candidates win slots. Never truncate silently.
      candidates = [
        ...candidates.filter((c) => c.corroborated),
        ...candidates.filter((c) => !c.corroborated),
      ].slice(0, maxCandidates);
      port.log(
        `Cap: cross-checking top ${maxCandidates} of ${byKey.size} candidates (corroborated first). Re-run with a narrower scope for the rest.`,
      );
    }
    ...
    port.phase('Cross-check');

    const checked = await port.parallel(
      'parallel-2',
      candidates.map(
        (c) => () =>
          ctx.claude
    ...
    port.log(
      `${confirmedDead.length} confirmed dead, ${kept.length} kept (living reference found or uncertain)`,
    );
    ...
    if (!apply || confirmedDead.length === 0) {
      return {
        ...
        kept: kept.map((k) => ({
          ...
          file: k.file,
          why: k.verdict
            ? k.verdict.referenceFound || k.verdict.reasoning
            : 'check failed — kept by default',
        })),
        applied: false,
      };
    }
   ...
   ${JSON.stringify(
     confirmedDead.map((c) => ({ kind: c.kind, identifier: c.identifier, file: c.file })),
     null,
     2,
   )}
   ...
   deletion. Do NOT fix failures by re-adding code — report them.`,
        ...
        // Original phase: 'Remove' — no matching ClaudeOptions control.
      })
      .then((result) => result.output);

    return {
      confirmedDead,
      kept: kept.map((k) => ({ identifier: k.identifier, file: k.file })),
      applied: true,
      verification,
    };
  }
