...
it('prefers turn.failed and unwraps nested API errors without trusting reconnect notices', () => {
  expect(
    ...
    kind: 'failure',
    failure: {
      reason: 'quota exceeded; notices: Reconnecting... 1/3; unrelated later notice',
      subtype: 'turn.failed',
      apiStatus: 429,
  ...
  expect(parseCodex('{"type":"error","message":"Reconnecting... 1/3"}')).toMatchObject({
    ...
    failure: {
      reason:
        'Codex output ended without turn.completed; the call may have been interrupted.; notices: Reconnecting... 1/3',
    },
  });
});
